...
class TestGPU(unittest.TestCase):
    ...
    def testBranch(self):
        self.runStandalone(Branch(Sum("x"), Average("2*x"), Deviate("3*x")).cuda(commentMain=False, testData=range(10)), {"data": {"data": [{"data": {"sum": 45.0, "name": "x", "entries": 10.0}, "type": "Sum"}, {"data": {"entries": 10.0, "name": "2*x", "mean": 9.0}, "type": "Average"}, {"data": {"variance": 74.25, "entries": 10.0, "name": "3*x", "mean": 13.5}, "type": "Deviate"}], "entries": 10.0}, "version": "0.9", "type": "Branch"})

    def testBranchNumpy(self):
        h = Branch(Sum("x"), Average("2*x"), Deviate("3*x"))
        if self.numpy is not None:
            h.pycuda(x = self.numpy.array(range(10)))
            self.assertEqual(h.toImmutable(), Factory.fromJson({"data": {"data": [{"data": {"sum": 45.0, "name": "x", "entries": 10.0}, "type": "Sum"}, {"data": {"entries": 10.0, "name": "2*x", "mean": 9.0}, "type": "Average"}, {"data": {"variance": 74.25, "entries": 10.0, "name": "3*x", "mean": 13.5}, "type": "Deviate"}], "entries": 10.0}, "version": "0.9", "type": "Branch"}))
