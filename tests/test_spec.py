    ...
    def runTest(self):
        ...
        try:
            testdata = json.load(
                reader(
                    urlopen(
                        "http://histogrammar.org/test/{0}/test-data.json".format(
                            histogrammar.version.specification
                        )
                    )
                )
            ...
            sys.stdout.write(
                "could not download http://histogrammar.org/test/{0}/test-data.json\nbecause of {1}: {2}\n".format(
        ...
        try:
            testresults = json.load(
                reader(
                    urlopen(
                        "http://histogrammar.org/test/{0}/test-results.json".format(
                            histogrammar.version.specification
                        )
                    )
                )
